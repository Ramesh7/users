...
action :create do
  ...
  if Chef::Config[:solo] and not chef_solo_search_installed?
    ...
    search(new_resource.data_bag, "groups:#{new_resource.search_group} AND NOT action:remove") do |u|
      ...
      is_user_active = u['expiration_date'] ? Time.parse(u['expiration_date']) >= Time.now : true
      is_action_create = u['action'] ? u['action'].eql?('create') : true
      custom_action = is_user_active && is_action_create ? :create : :remove
      security_group << u['username'] if custom_action.to_s.eql?('create')
      ...
      next unless is_user_active && is_action_create
      ...
      if u['expiration_date']
        template "#{home_dir}/.bash_profile" do
          ...
          group u['username']
          mode "0644"
          variables(remaining_time: (Time.parse(u['expiration_date']) - Time.now).to_i)
        end
      end
